...
class StockPriceService {  
  ...
  static async fetchQuoteWithRetry(ticker, retryCount = 0) {
    try {
      console.log(`[${ticker}] Fetching quote data`);
      
      // Add delay before each request to avoid rate limiting
      await this.sleep(5000); // Increased base delay for quotes
      ...
      console.error(`[${ticker}] Error fetching quote:`, error.message);
      ...
      if (shouldRetry && retryCount < 3) {
        const waitTime = Math.pow(2, retryCount) * 60000; // 1-minute base wait time
        console.log(`[${ticker}] Will retry quote fetch in ${waitTime/1000} seconds (Attempt ${retryCount + 1}/3)`);
        await this.sleep(waitTime);
  ...
  static async updateHistoricalDataForAllCompanies() {  
    try {  
      ...
      const batchSize = 15; // Reduced to just 2 companies per batch
  ...
  static async processDailyPriceBatch(companies, today) {
    const results = [];
    console.log(`Processing daily price batch of ${companies.length} companies`);
    
    for (const company of companies) {
      try {
        console.log(`[${company.ticker}] Fetching daily data`);
        
        const quote = await this.fetchQuoteWithRetry(company.ticker);
        ...
        await this.updateOrCreateStockPrice({
          ...
          adjClose: quote.regularMarketPrice // For current day, adjusted close equals close
        });
