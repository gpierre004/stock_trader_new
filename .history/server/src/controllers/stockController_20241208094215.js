...
const stockController = {
    ...
    // Updated method for market data update
    updateMarketData: async (req, res) => {
        try {
            const { historical } = req.query;
            let stockPriceUpdateResult;

            if (historical === 'true') {
                // Update historical data if requested
                stockPriceUpdateResult = await StockPriceService.updateHistoricalDataForAllCompanies();
            } else {
                // Otherwise just update daily prices
                stockPriceUpdateResult = await StockPriceService.updateStockPrices();
            }
