...
exports.createTransaction = async (req, res) => {
  const t = await db.sequelize.transaction();
  try {
    const { ticker, quantity, purchase_price, type, purchase_date, account_id } = req.body;

    if (!ticker || !quantity || !purchase_price || !type || !purchase_date || !account_id) {
      return res.status(400).json({ 
        error: 'Missing required fields. Please provide ticker, quantity, purchase_price, type, purchase_date, and account_id.' 
      });
    }
