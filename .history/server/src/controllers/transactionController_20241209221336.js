  ...
  try {
    ...
    const transformedData = data.map(row => {
      return {
        ...
        // Extract Type from the description field which contains "YOU BOUGHT" or "YOU SOLD"
        Type: row.comment?.includes('BOUGHT') ? 'BUY' : 'SELL'
      };
    });
    ...
    const missingColumns = requiredColumns.filter(col => !(col in firstRow) || firstRow[col] === undefined);
