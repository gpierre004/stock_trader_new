    ...
    for (const row of transformedData) {
      try {
        ...
        await db.CashTransaction.create({
          user_id: transaction.portfolio_id,  // Use portfolio_id instead of user.id
...
exports.syncCashTransactions = async (req, res) => {
  ...
  try {
    ...
    for (const transaction of transactions) {
      ...
      // Parse the description to determine if it's a buy or sell
      const isBuy = transaction.description?.toLowerCase().includes('buy') || 
      ...
      await db.CashTransaction.create({
        ...
        amount: isBuy ? -cashAmount : cashAmount,
        balance_after: currentBalance,
        related_stock_transaction_id: transaction.purchase_id,  // Use purchase_id instead of id
