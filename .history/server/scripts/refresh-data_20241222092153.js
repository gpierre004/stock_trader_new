...
async function refreshData() {
    try {
        ...
        const sp500Result = await axios.post(
            `${API_URL}/api/maintenance/refresh-sp500`,
            {},
            config
        );
        ...
        const syncResult = await axios.post(
            `${API_URL}/api/maintenance/sync-missing-companies`,
            {},
            config
        );
        ...
        const historyResult = await axios.post(
            `${API_URL}/api/maintenance/update-full-stock-history`,
            {},
            config
        );
        ...
        console.error('Error during data refresh:', error.response?.data || error.message);
    }
}
