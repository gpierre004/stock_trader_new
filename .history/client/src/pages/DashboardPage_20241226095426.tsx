...
const DashboardPage: React.FC = () => {
  const { data: marketData, isLoading: marketLoading } = useQuery({
    ...
    queryFn: () => api.get('/api/market-data/overview').then(res => res.data)
  });

  const { data: bullishData, isLoading: bullishLoading } = useQuery({
    queryKey: ['bullishStocks'],
    queryFn: () => api.get('/api/market-data/bullish-stocks').then(res => res.data)
  });

  const { data: signalsData, isLoading: signalsLoading } = useQuery({
    queryKey: ['buySellSignals'],
    queryFn: () => api.get('/api/market-data/buy-sell-signals').then(res => res.data)
  });
