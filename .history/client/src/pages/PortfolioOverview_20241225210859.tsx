...
const PortfolioOverview: React.FC = () => {
  ...
  const { data: portfolioHoldings } = useQuery({
    ...
    queryFn: () => api.get('/transactions/holdings').then(res => res.data)
  });

  const { data: portfolioTickers = [] } = useQuery({
    queryKey: ['portfolioTickers'],
    queryFn: () => api.get('/transactions/tickers').then(res => res.data)
  });

  // Then fetch data filtered by those tickers
  const { data: marketData, isLoading: marketLoading } = useQuery({
    queryKey: ['portfolioMarketOverview', portfolioTickers],
    queryFn: () => api.get(`/market-data/overview?tickers=${portfolioTickers.join(',')}`).then(res => res.data),
  ...
  const { data: bullishData, isLoading: bullishLoading } = useQuery({
    ...
    queryFn: () => api.get(`/market-data/bullish-stocks?tickers=${portfolioTickers.join(',')}`).then(res => res.data),
    enabled: portfolioTickers.length > 0
  });

  const { data: signalsData, isLoading: signalsLoading } = useQuery({
    queryKey: ['portfolioBuySellSignals', portfolioTickers],
    queryFn: () => api.get(`/market-data/buy-sell-signals?tickers=${portfolioTickers.join(',')}`).then(res => res.data),
    enabled: portfolioTickers.length > 0
  });
