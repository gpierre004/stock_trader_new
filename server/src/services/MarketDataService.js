...
class MarketDataService {
  ...
  async getBullishStocks(tickers = null) {
    try {
      ...
      const prices = await StockPrice.findAll({
        attributes: ['ticker', 'date', 'close', 'volume'],
        where: whereClause,
        order: [['date', 'DESC']],
        raw: true
      });
      ...
      prices.forEach(price => {
        ...
        stocksMap.get(price.ticker).prices.push({
          ...price,
          close: Number(price.close),
          volume: Number(price.volume)
        });
      ...
      console.log(`Processing ${stocksMap.size} unique tickers`);
      ...
      stocksMap.forEach((data, ticker) => {
        ...
        results.push({
          ticker,
          latest_close: Number(latestClose),
          close_30d_ago: Number(avg30DClose),
          close_90d_ago: Number(min90DClose),
      ...
      console.log(`Returning ${results.length} bullish stocks results`);
      return results;
    } catch (error) {
      console.error('Error in getBullishStocks:', error);
  ...
  async getMomentumAnalysis(tickers = null) {
    try {
      ...
      const prices = await StockPrice.findAll({
        attributes: ['ticker', 'date', 'close', 'high', 'low'],
        where: whereClause,
        order: [['date', 'DESC']],
        raw: true
      });
      ...
      stocksMap.forEach((data, ticker) => {
        ...
        results.push({
          ticker,
          latest_close: Number(latest.close),
          start_close_30d: Number(minClose),
          end_close_30d: Number(latest.close),
          min_close_30d: Number(minLow),
          max_close_30d: Number(maxHigh),
